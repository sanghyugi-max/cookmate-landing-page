...
(() => {
  ...
  app.innerHTML = `
    ...
    ${section('specs', c.specs, `${modelCards()}<div class="table-wrap"><table class="specs"><tbody>${c.specs.rows.map(r => `<tr><th scope="row">${e(r.label)}</th><td>${r.placeholder ? badge() : ''}<span>${e(r.value)}</span></td></tr>`).join('')}</tbody></table></div>`)}
